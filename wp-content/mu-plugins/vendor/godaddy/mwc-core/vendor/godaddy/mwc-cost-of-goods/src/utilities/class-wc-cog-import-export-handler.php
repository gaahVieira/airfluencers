...
namespace GoDaddy\WordPress\MWC\CostOfGoods\Utilities;

defined( 'ABSPATH' ) or exit;

use SkyVerge\WooCommerce\CSV_Export\XML_Export_Generator;
use SkyVerge\WooCommerce\PluginFramework\v5_11_0 as Framework;
use WC_Customer_Order_CSV_Export_Generator;
use WC_Order;
...
class WC_COG_Import_Export_Handler {
	...
	public function __construct() {
		...
		add_filter( 'woocommerce_csv_product_post_columns', [ $this, 'add_cost_to_csv_product_export_columns' ] );
		add_filter( 'woocommerce_csv_product_variation_post_columns', [ $this, 'add_cost_to_csv_product_export_columns' ] );

		// Product CSV Import Suite: import support
		add_action( 'woocommerce_csv_product_data_mapping', [ $this, 'add_cost_to_csv_import_mapping' ] );
		add_filter( 'woocommerce_csv_product_postmeta_defaults', [ $this, 'add_cost_to_csv_import_postmeta_defaults' ] );
	...
	public function add_cost_to_csv_export_column_headers( $headers, $csv_generator ) {
		...
		if ( $this->is_one_row_per_item( $csv_generator ) ) {
			...
			if ( isset( $headers['item_total'] ) ) {
				$headers = Framework\SV_WC_Helper::array_insert_after( $headers, 'item_total', [ 'item_cost' => 'item_cost' ] );
			} else {
	...
	public function add_cost_to_csv_export_column_data( $order_data, $order, $csv_generator ) {
		...
		$cost_data  = [ 'order_cost_total' => ! empty( $total_cost ) ? $total_cost : '' ];
	 ...
	 * @param XML_Export_Generator $generator export generator
	 * @return array
	 */
