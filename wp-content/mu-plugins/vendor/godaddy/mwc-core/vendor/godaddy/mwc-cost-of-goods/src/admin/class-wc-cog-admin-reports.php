...
class WC_COG_Admin_Reports {
	...
	public function __construct() {
		...
		add_action( 'woocommerce_delete_shop_order_transients', [ $this, 'clear_report_transients' ] );

		// calculate total valuation in batching
		add_action( 'wp_ajax_wc_cog_do_ajax_total_valuation', [ $this, 'calculate_total_valuation' ] );

		// calculate product valuation in batching
		add_action( 'wp_ajax_wc_cog_do_ajax_product_valuation', [ $this, 'calculate_product_valuation' ] );
	}
	...
	public function load_report( $name ) {
		...
		// then report class
		$report = wc_cog()->load_class( "/src/admin/reports/$filename", '\GoDaddy\WordPress\MWC\CostOfGoods\Admin\Reports\WC_COG_Admin_Report_' . $name );
	...
	public function clear_report_transients() {
		...
		foreach ( [ 'date', 'product', 'category' ] as $report ) {
