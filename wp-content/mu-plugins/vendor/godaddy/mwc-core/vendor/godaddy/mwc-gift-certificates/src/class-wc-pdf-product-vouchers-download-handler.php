...
class MWC_Gift_Certificates_Download_Handler {
	...
	public function __construct() {
		...
		if ( ! empty( $_GET['download_wc_voucher_png'] ) ) {
			add_action( 'init', array( $this, 'download_voucher_png' ) );
		}
	...
	public function download_voucher_pdf() {
		$this->download_voucher();
	}

	/**
	 * Download the PNG preview of a generated PDF.
	 *
	 * @return void
	 */
	public function download_voucher_png() {
		$this->download_voucher( 'download_wc_voucher_png', 'png' );
	}

	/**
	 * Download a voucher
	 *
	 * @param string $id_key The `$_GET` key containing the voucher ID (default: download_wc_voucher_pdf).
	 * @param string $type The filetype to download (default: pdf).
	 * @return void
	 */
	public function download_voucher($id_key = 'download_wc_voucher_pdf', $type = 'pdf') {

		$voucher = wc_pdf_product_vouchers_get_voucher( $_GET[$id_key] );
		...
		$file_path = $voucher->get_voucher_full_filename( $type );
